...
def calculate_bandwidth(events, time_elapsed):
    """
    Calculates memory bandwidth in GB/s from raw perf event counts for AMD systems.
    ...
    try:
        ...
        # Read Bandwidth
        read_bytes = events.get('amd_umc/umc_cas_cmd.rd/', 0) * BYTES_PER_CAS
        metrics['Read_Bandwidth_GBs'] = safe_divide(read_bytes, time_elapsed * GIGABYTE)
        
        # Write Bandwidth
        write_bytes = events.get('amd_umc/umc_cas_cmd.wr/', 0) * BYTES_PER_CAS
        metrics['Write_Bandwidth_GBs'] = safe_divide(write_bytes, time_elapsed * GIGABYTE)
        
        # Total Bandwidth
        total_bytes = events.get('amd_umc/umc_cas_cmd.all/', 0) * BYTES_PER_CAS
...
def main():
    """
    Main function to process all CSV files with bandwidth patterns.
    """
    input_dir = "Raw_Files"
    ...
    # Find all CSV files with bandwidth patterns
    is_amdc_pattern = os.path.join(input_dir, "AMDC_AMDS_bandwidth*.csv")
    #ic_amds_pattern = os.path.join(input_dir, "AMD_AMDS_bandwidth*.csv")
    
    is_amdc_files = glob.glob(is_amdc_pattern)
    #ic_amds_files = glob.glob(ic_amds_pattern)
    csv_files = sorted(is_amdc_files)
