...
def calculate_bandwidth(events, time_elapsed):
    ...
    try:
        # Read Bandwidth
        read_bytes = events.get('unc_m_cas_count_rd', 0) * BYTES_PER_CAS
        metrics['Read_Bandwidth_GBs'] = safe_divide(read_bytes, time_elapsed * GIGABYTE)
        
        # Write Bandwidth
        write_bytes = events.get('unc_m_cas_count_wr', 0) * BYTES_PER_CAS
...
def main():
    ...
    is_amdc_pattern = os.path.join(input_dir, "IC_IS_bandwidth*.csv")
    ...
    # Write summary file
    if all_metrics:
        summary_file = os.path.join(output_dir, "bandwidth_metrics_summary_ic.csv")
