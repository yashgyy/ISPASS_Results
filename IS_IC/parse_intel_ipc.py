...
def calculate_ipc(events, time_elapsed):
    ...
    try:
        ...
        # Cycles count 
        cycles = events.get('cpu-cycles', 0)
...
def main():
    ...
    is_amdc_pattern = os.path.join(input_dir, "IS_IC_ipc*.csv")
    ic_amdc_pattern = os.path.join(input_dir, "IS_IS_ipc*.csv")
    ...
    # Write summary file
    if all_metrics:
        summary_file = os.path.join(output_dir, "ipc_metrics_summary.csv")
