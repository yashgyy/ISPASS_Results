...
def parse_and_calculate_metrics(input_file_path):
    ...
    try:
        ...
        ic_loads = events.get('icache_64b.iftag_hit', 0) + events.get('icache_64b.iftag_miss', 0)
        metrics['IC_Miss_Percentage'] = safe_divide(events.get('L1-icache-load-misses', 0), ic_loads)

        # L1 Data Cache Miss Percentage
        metrics['DC_Miss_Percentage'] = safe_divide(events.get('L1-dcache-load-misses', 0), events.get('L1-dcache-loads', 0))

        # L2 Miss Percentage
        metrics['L2_Miss_Percentage'] = safe_divide(events.get('l2_rqsts.miss', 0), events.get('l2_rqsts.references', 0))

        # L3 Miss Percentage
        metrics['L3_Miss_Percentage'] = safe_divide(events.get('LLC-load-misses', 0), events.get('LLC-loads', 0))

        # L2 Miss Rate from IC Misses
        metrics['L2_Miss_Rate_from_IC_Miss'] = safe_divide(events.get('l2_rqsts.code_rd_miss', 0), events.get('L1-icache-load-misses', 0))

        # L2 Miss Rate from DC Misses
        metrics['L2_Miss_Rate_from_DC_Miss'] = safe_divide(events.get('l2_rqsts.demand_data_rd_miss', 0), events.get('L1-dcache-load-misses', 0))
...
def main():
    ...
    is_amdc_pattern = os.path.join(input_dir, "IS_AMDC_micro*.csv")
    ic_amdc_pattern = os.path.join(input_dir, "IC_AMDS_micro*.csv")
    ...
    # Write summary file
    if all_metrics:
        summary_file = os.path.join(output_dir, "perf_metrics_summary.csv")
